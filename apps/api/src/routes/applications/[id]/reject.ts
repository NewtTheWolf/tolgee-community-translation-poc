...
export default new Elysia().use(authMiddleware).post('/applications/:id/reject', async ({ params, user, status }) => {
  ...
  await db.update(applications).set({ status: 'rejected', reviewedBy: user.id, reviewedAt: new Date() }).where(eq(applications.id, app.id))
  await writeAudit({ actorUserId: user.id, action: 'application.reject', targetType: 'application', targetId: app.id, meta: { locale: app.locale, role: app.requestedRole } })
  return { ok: true }
})
